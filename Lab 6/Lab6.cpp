...
public:
    ...
    // overloaded function
    void setData(int hW);
    void setData(double dW);
    
    // operator overloading
    friend WorkerHours operator+(WorkerHours a, WorkerHours b);
    friend WorkerHours& operator+=(WorkerHours &a, const WorkerHours &b);
...
void WorkerHours::setData(int hW)
{
    hoursWorked = hW;
}
...
// function.
void WorkerHours::setData(double dW)
{
    setData(static_cast<int>(dW * 24));     // multiplication by 24 
...
// defining the (+=) overloaded operator
WorkerHours& operator+=(WorkerHours &a, const WorkerHours &b)   // note that a is passed by reference
{
    a.hoursWorked += b.hoursWorked;                 // updates a.hoursWorked
    //return WorkerHours(a.hoursWorked);
    return a;
}
...
{
    double temp;
    temp = static_cast<double>(hoursWorked) / 24.0;
    return temp;
}
